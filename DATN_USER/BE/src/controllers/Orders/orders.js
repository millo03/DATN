...
export const createOrderPayment = async (req, res) => {
  try {
    const {
      ...
      customerInfo,
      totalPrice,
      discountCode = null, // Mã giảm giá (nếu có)
      discountAmount = 0, // Số tiền giảm giá (nếu có)
    ...
    dataCart.products = dataCart.products.filter((i) => {
      ...
      return !foundItem; // Loại bỏ sản phẩm đã đặt hàng
    });
...
export const getAllTemporaryOrder = async (req, res) => {
  try {
    const startOfDay = new Date();
    startOfDay.setHours(0, 0, 0, 0);
    const endOfDay = new Date();
    endOfDay.setHours(23, 59, 59, 999);
    const temporaryOrders = await Order.find({
      createdAt: {
        $gte: startOfDay,
        $lte: endOfDay
      }
    ...
    return res.status(StatusCodes.OK).json(temporaryOrders);
  } catch (error) {
    return res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
      .json({ error: error.message });
...
//     const endOfDay = new Date();
//     endOfDay.setHours(23, 59, 59, 999);
//     console.log("Ngày bắt đầu:", startOfDay);
//     console.log("Ngày kết thúc:", endOfDay);
//     const ordersToday = await Order.find({
//       createdAt: {
//         $gte: startOfDay,
//         $lte: endOfDay
...
//       .limit(10)
//       .exec();
//     console.log("10 đơn hàng trong ngày ", ordersToday);
//     if (!ordersToday || ordersToday.length === 0) {
//       return res
//         .status(StatusCodes.NOT_FOUND)
//         .json({ message: "Đơn hàng không tìm thấy!" });
//     }
//     return res.status(StatusCodes.OK).json(ordersToday);
//   } catch (error) {
